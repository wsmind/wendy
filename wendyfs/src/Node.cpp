...
std::vector<std::string> Node::list()
{
...
Node *Node::find(const std::string &path)
{
	if (path == "")
		return this;
...
void Node::insert(const std::string &path, const std::string &id)
{
	std::string childName;
	std::string childPath;
	this->extractPathComponents(path, &childName, &childPath);
	...
	{
		// leaf node creation
		this->subNodes[childName] = new Node(id);
	}
	else
	{
		...
		NodeMap::iterator it = this->subNodes.find(childName);
		if (it == this->subNodes.end())
			this->subNodes[childName] = new Node;
...
void Node::extractPathComponents(const std::string &path, std::string *childName, std::string *childPath)
{
